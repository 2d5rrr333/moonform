...
test "doc: lens read/write" {
  ...
  inspect(nickname_l().get(p), content=(
    #|moon
  ))
...
test "doc: composed key derives from both segments" {
  fn inner_l() -> Lens[Outer, Inner] {
    field("inner", o => o.inner, (o, i) => { ..o, inner: i })
  }
  fn city_l() -> Lens[Inner, String] {
    field("city", i => i.city, (i, c) => { ..i, city: c })
  }
...
test "doc: form lifecycle" {
  ...
  inspect(form.get_value(login_email_l()), content=(
    #|
  ))
...
fn username_l() -> Lens[Signup, String] {
  field("username", s => s.username, (s, u) => { ..s, username: u })
}
...
test "doc: onChange validation fills the error slot" {
  ...
  let username = form.field(
    username_l(),
    on_change_validate=validator(
      v => if v.length() < 3 { ["min 3 chars"] } else { [] },
    ),
  )
  username.mount()
  username.set_value("ab")
  inspect(username.meta().errors(), content=(
    #|[min 3 chars]
  ))
...
fn friends_l() -> Lens[Roster, Array[Friend]] {
  field("friends", r => r.friends, (r, xs) => { ..r, friends: xs })
}

fn friend_name_l() -> Lens[Friend, String] {
  field("name", f => f.name, (f, n) => { ..f, name: n })
}
...
test "doc: remove migrates meta to shifted slots" {
  ...
  .meta_store()
  .update(
    friends_l().key.push_idx(1).push_field("name"),
    m => m.with_cause_errors(ValidationCause::change(), Some(["taken"])),
  )
  ...
  inspect(form.field_meta(moved).errors(), content=(
    #|[taken]
  ))
}
```
...
fn token_l() -> Lens[AsyncForm, String] {
  field("token", f => f.token, (f, t) => { ..f, token: t })
}
...
test "doc: debounced async validation on the virtual clock" {
  ...
  let token = form.field(
    ...
    on_change_async_validate=async_validator(
      (value, clock, done) => {
  ...
  inspect(token.meta().errors(), content=(
    #|[token already taken]
  ))
}
```
...
test "doc: submit gates on validation" {
  ...
  let qty = form.field(
    qty_l(),
    on_submit_validate=validator(v => if v < 1 { ["qty must be >= 1"] } else { [] }),
  )
  qty.mount()
