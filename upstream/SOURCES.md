...
`moon package` 打包整个模块目录（无发布排除机制），因此 mooncakes 包内含
`upstream/`（本目录）、`web/`（浏览器 demo 及其 React UMD vendor）、`tools/`
（无头验证脚本）——约 1MB 与库本体无关的材料。这是有意权衡：三者均为终审
要求的可复现证据（README demo 复现 / 浏览器验证 / 许可溯源），移出模块根会
破坏仓库自包含性。库本体的导入面不受影响（`source = "src"`）。
