...
> 用途：新会话打开时把本文件发给编程助手，即可无缝接续。最后更新：2026-09-25（0.7.1）。
...
另：README.mbt.md 的 doc 测试块标记若被破坏会**静默**不编译不计数（` ```mbt check ` 三反引号），改动后务必核对测试总数（js 292）。
...
```bash
cd moonform
moon test --target js --deny-warn       # 292
moon test --target wasm --deny-warn     # 275
