...
| `rules` | none | required/min/max/length/pattern/contains/equals/one_of/non_blank/numeric/must_be_true/custom closures |
...
```bash
moon test --target js      # 322 tests (incl. upstream translations + doc-tests)
moon test --target wasm    # 298 tests
