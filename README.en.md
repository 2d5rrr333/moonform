...
| `rules` | none | required/min/max/length/pattern/contains/custom closures |
...
```bash
moon test --target js      # 198 tests (incl. upstream translations)
moon test --target wasm    # same suite on wasm
moon test --target native  # native (needs a local C compiler)
