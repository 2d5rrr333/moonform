...
| 发布 | mooncakes.io `2d5rrr333/moonform@0.8.3`（0.1.0 → 0.8.3 共 13 版） |
...
- **优势**（对比生态外方案）：
  ...
  - 行为正确性不靠自夸：上游 342 个测试用例的**行为等价翻译**做验收规格
    （185 个译文 + 豁免/偏离对账，见 upstream/PARITY.md）
...
3. **行为等价翻译做验收**：不逐行翻译，把上游测试集翻译成 MoonBit 测试
   （185 篇译文），豁免与偏离逐条记录在案（PARITY.md）——可审计、可复现
...
- 测试 317（js）/ 293（wasm）全绿；native CI 通过；三目标 `moon check --deny-warn`
...
- 已发布：mooncakes.io `2d5rrr333/moonform@0.8.3`
